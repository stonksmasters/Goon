...
const MemeCanvas = forwardRef((props, ref) => {
  ...
  const handleLayerUp = () => {
    ...
    if (activeObject) {
      ...
      const objAbove = fabricCanvas.getObjects().find(obj => obj.layerNumber === activeObject.layerNumber + 1 && obj.category === 'Main');
      if (objAbove) {
  ...
  const handleLayerDown = () => {
    ...
    if (activeObject) {
      ...
      // Find the object with layerNumber = activeObject.layerNumber -1
      const objBelow = fabricCanvas.getObjects().find(obj => obj.layerNumber === activeObject.layerNumber - 1 && obj.category === 'Main');
      if (objBelow) {
...
const handleSave = () => {
  ...
  <div className="flex-1 flex flex-col items-center">
    <div
      className={`relative border-4 rounded-lg p-2 bg-grey-02 transition-all duration-200 overflow-hidden ${
        isDragOver ? 'border-orange bg-black-03' : 'border-transparent'
      }`}
    >
      <div className="relative w-[300px] h-[300px] md:w-[450px] md:h-[450px] lg:w-[600px] lg:h-[600px] overflow-hidden">
        <canvas
          ref={canvasElementRef}
          className="w-full h-full border-2 border-dashed border-orange rounded-lg z-10"
          width={600}
          height={600}
        />
        {isDragOver && (
